...
export function formatNumber(v: number, maxPrecision = 3, minPrecision = 0, useSpaces?: boolean) {
    // Making sure we hae the coma
...
export function fromCamelCase(s: string, capitalize: "yes" | "no" | "leaveAsIs" = "yes"): string {
    let result = "";
    let newWord = true;
...
function parseStringToDateWithFormatCheck<T>(s: string | null | undefined, f: (s: string) => boolean): Date | null {
...
export function encodeToCsv(s: any, decimalSeparator = '.'): string {
    if (typeof s === "string") {
        ...
        // Due to localization in Germmany, we change decimalSeparator. By default it is "." 
        ...
        return encodeToCsv(time)
    }
    else if (typeof s === 'object') {
        return encodeToCsv(JSON.stringify(s))
    }
    else {
        return encodeToCsv("" + s);
    }
}
